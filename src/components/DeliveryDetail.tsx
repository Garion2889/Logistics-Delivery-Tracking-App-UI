...
interface Delivery {
  id: string;
  ref_no: string;          // Changed from refNo
  customer_name: string;   // Changed from customer
  delivery_address: string; // Changed from address
  status: string;
  assigned_driver?: string; // This will likely be an ID or Name depending on your fetch
  
  // These come from the SQL View
...
export function DeliveryDetail({ delivery, onClose }: DeliveryDetailProps) {
  ...
  const getIconForLabel = (label: string) => {
    switch (label) {
      ...
      case "Picked Up": return Package; // Or a different icon like BoxSelect
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              <div className="space-y-6">
                {/* Fallback if timeline_events is missing */}
                {(!delivery.timeline_events || delivery.timeline_events.length === 0) && (
                   <div className="flex items-center gap-2 text-yellow-600 bg-yellow-50 p-3 rounded">
                      ...
                      <span className="text-sm">Timeline data not available (Check SQL View)</span>
                   </div>
                )}
                  ...
                  return (
                    <div key={index} className="flex gap-4">
                      ...
                      <div className="flex-1 pb-8">
                        ...
                        {/* Render the pre-formatted date from DB */}
